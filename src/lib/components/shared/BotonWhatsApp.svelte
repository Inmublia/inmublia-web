    ...
    try {
      ...
      if (typeof window.gtag === 'function' && broker?.pixel_google) {
        window.gtag('event', 'generate_lead', {
          currency: propiedad?.moneda || 'MXN',
          value: propiedad?.precio || 0,
          item_id: propiedad?.id,
          item_name: propiedad?.titulo
        });
      }
...
{#if broker?.whatsapp}
  <a 
    href="https://wa.me/{broker.whatsapp}?text=Hola,%20me%20interesa%20agendar%20un%20recorrido%20cinematográfico%20para:%20{propiedad?.titulo}" 
    target="_blank" 
    class="fixed bottom-8 right-8 bg-white/5 backdrop-blur-md border border-white/20 text-white p-4 rounded-full shadow-[0_0_40px_rgba(0,0,0,0.8)] hover:scale-110 hover:bg-white/10 transition-all duration-500 z-[100] flex items-center justify-center group" 
    aria-label="Contactar por WhatsApp"
    onclick={registrarLeadMetaYTikTok}
  >
