...
{#if !isBrochure}
  <a href="https://wa.me/{broker.whatsapp}?text=Hola,%20solicito%20acceso%20privado%20a:%20{propiedad.titulo}" target="_blank" class="fixed bottom-8 right-8 bg-white/10 backdrop-blur-xl border border-white/20 text-white p-4 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.5)] hover:scale-110 hover:bg-white/20 transition-all duration-300 z-[100] flex items-center justify-center group" aria-label="Contactar por WhatsApp">
...
<main class="relative z-10 w-full flex flex-col font-sans">
  ...
  <nav class="fixed top-0 w-full z-40 bg-gradient-to-b from-black/80 to-transparent pt-8 pb-16 pointer-events-none">
    <div class="max-w-[1400px] mx-auto px-6 md:px-12 flex justify-between items-center pointer-events-auto">
      ...
      {#if isBrochure}
        <span class="bg-black/40 backdrop-blur-xl text-white text-[9px] font-bold uppercase tracking-[0.2em] px-4 py-2 border border-white/10 rounded-full shadow-2xl flex items-center gap-2">
          <Sparkles class="w-3 h-3" /> Elite Access
        </span>
      {:else}
    ...
    <div class="max-w-[1400px] w-full mx-auto px-6 md:px-12">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div class="text-left md:text-right">
          ...
          <p class="text-4xl md:text-6xl font-black text-white drop-shadow-2xl">{formatearPrecio(propiedad.precio)}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="w-full {isNight ? 'bg-zinc-950/80 border-t border-zinc-800' : 'bg-white/85 border-t border-white'} glass-panel shadow-[0_-20px_60px_rgba(0,0,0,0.3)] pb-32">
    <div class="max-w-[1400px] mx-auto px-6 md:px-12 pt-16">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-24 border-b {isNight ? 'border-zinc-800/50' : 'border-slate-300/50'} pb-16">
      ...
      {#if !isBrochure}
        <div class="max-w-4xl mx-auto rounded-[2.5rem] p-10 md:p-16 border shadow-2xl {isNight ? 'bg-zinc-900/80 border-zinc-700/50' : 'bg-white/80 border-white'} glass-panel">
          ...
          <div class="mt-12 pt-8 border-t flex flex-col items-center {isNight ? 'border-zinc-800' : 'border-slate-300'}">
            ...
            <button onclick={descargarVCard} class="mt-8 text-[9px] font-bold uppercase tracking-[0.2em] border px-6 py-2.5 rounded-full transition-all flex items-center gap-2 {isNight ? 'border-zinc-700 text-zinc-400 hover:bg-zinc-800 hover:text-white' : 'border-slate-300 text-slate-500 hover:bg-slate-100 hover:text-slate-900'}">
