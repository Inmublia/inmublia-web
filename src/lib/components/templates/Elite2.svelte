<script>
  ...
  import { 
    ...
    CheckCircle2, AlertCircle, MapPin, Compass, Send, Download
  } from 'lucide-svelte';
  
  // 🔥 Utilidades y componentes compartidos Svelte 5
...
{#if isGalleryOpen}
  <div class="fixed inset-0 z-[200] bg-white/95 backdrop-blur-2xl flex items-center justify-center animate-in fade-in duration-300">
    <button class="absolute top-6 right-6 text-slate-900 bg-slate-100 p-4 rounded-full z-[210]" onclick={closeGallery}><X class="w-6 h-6" /></button>
    <button class="absolute left-6 text-slate-900 bg-slate-100 p-4 rounded-full z-[210]" onclick={prevImage}><ChevronLeft class="w-8 h-8" /></button>
    ...
    <button class="absolute right-6 text-slate-900 bg-slate-100 p-4 rounded-full z-[210]" onclick={nextImage}><ChevronRight class="w-8 h-8" /></button>
  </div>
{/if}
...
<main class="relative w-full bg-slate-50 font-sans text-slate-900 overflow-x-hidden selection:bg-indigo-600 selection:text-white">
  ...
  <div class="fixed inset-0 w-full h-screen z-0 bg-black pointer-events-none">
    {#if videoId}
      <div class="absolute inset-0 w-full h-full overflow-hidden opacity-70">
        <iframe src="https://www.youtube.com/embed/{videoId}?autoplay=1&mute=1&controls=0&showinfo=0&rel=0&loop=1&playlist={videoId}&modestbranding=1&iv_load_policy=3" class="video-bg" frameborder="0" allow="autoplay; encrypted-media"></iframe>
      </div>
  ...
  <div class="relative z-10 w-full h-screen flex flex-col justify-end pb-12 md:pb-24">
    <div class="max-w-[1400px] w-full mx-auto px-6 md:px-12 flex flex-col items-center text-center">
      ...
      <p class="text-3xl md:text-4xl font-light text-white/90 drop-shadow-xl">{formatearPrecio(propiedad.precio, moneda)}</p>
      <div class="mt-16 animate-bounce"><span class="block w-px h-12 bg-gradient-to-b from-white to-transparent mx-auto"></span></div>
    </div>
  </div>
    ...
    <div class="max-w-[1400px] mx-auto px-6 md:px-12 pt-20">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 mb-24">
        <div class="lg:col-span-5">
          ...
          <div class="prose prose-lg max-w-none font-light leading-relaxed whitespace-pre-line text-slate-700">{propiedad.descripcion}</div>
        </div>
                ...
                <div class="relative overflow-hidden cursor-pointer rounded-3xl group {idx === 0 ? 'sm:col-span-2 aspect-[21/9]' : 'aspect-square'}" role="button" tabindex="0" onclick={() => openGallery(idx)}>
                  ...
                  {#if idx === 3 && allPhotos.length > 4}
                    <div class="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center">
                      <span class="text-slate-900 font-bold text-[10px] tracking-[0.3em] uppercase border border-slate-900/20 px-6 py-2 rounded-full">+ {allPhotos.length - 4} Vistas</span>
                    </div>
                  {/if}
                </div>
              {/each}
      ...
      <div class="max-w-3xl mx-auto mt-32">
        <div class="bg-[#1a1a1e] border border-white/5 p-12 md:p-16 text-center backdrop-blur-xl rounded-[2rem] shadow-2xl">
          ...
          {#if form?.success}
            <div class="bg-emerald-950/50 text-emerald-400 p-6 rounded-2xl font-bold mb-8 flex items-center justify-center gap-3 animate-bounce border border-emerald-500/30"><CheckCircle2 class="w-5 h-5" /> ¡Solicitud enviada! Te contactaremos pronto.</div>
          {:else if form?.error}
      ...
      <div class="mt-40 pt-20 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-12 bg-[#1a1a1e] rounded-[3rem] p-16 shadow-2xl border border-white/5">
        <div class="flex items-center gap-8">
          ...
          <div>
            ...
            <button onclick={() => descargarVCardAgente(broker, propiedad.ubicacion)} class="mt-4 text-xs font-bold uppercase tracking-widest text-emerald-400 hover:text-emerald-300 transition-colors flex items-center gap-2"><Download class="w-4 h-4" /> Descargar vCard</button>
          </div>
        </div>
        
        <div class="flex items-center gap-4">
          {#if broker.facebook}<a href={broker.facebook} target="_blank" class="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-900/80 border border-white/5 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Facebook class="w-5 h-5" /></a>{/if}
          {#if broker.instagram}<a href={broker.instagram} target="_blank" class="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-900/80 border border-white/5 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Instagram class="w-5 h-5" /></a>{/if}
          {#if broker.linkedin}<a href={broker.linkedin} target="_blank" class="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-900/80 border border-white/5 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Linkedin class="w-4 h-4" /></a>{/if}
          {#if broker.tiktok}<a href={broker.tiktok} target="_blank" class="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-900/80 border border-white/5 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all group"><img src="https://cdn.simpleicons.org/tiktok/94a3b8/ffffff" alt="TikTok" class="w-5 h-5 transition-all group-hover:scale-110" /></a>{/if}
        </div>
      </div>
