<script>
  ...
  // 🔥 TRACKING MANUAL PARA EL BOTÓN NATIVO EN EL SIDEBAR
  function registrarLeadWhatsAppSecundario(e) {
    e.preventDefault(); 
    const url = e.currentTarget.href;
    try {
      if (typeof window.fbq === 'function' && broker?.pixel_fb) {
        window.fbq('track', 'Lead', { content_name: propiedad.titulo, currency: moneda, value: propiedad.precio });
      }
      if (typeof window.gtag === 'function' && broker?.pixel_google) {
        window.gtag('event', 'generate_lead', { currency: moneda, value: propiedad.precio, item_id: propiedad.id, item_name: propiedad.titulo });
      }
      if (typeof window.ttq === 'object' && typeof window.ttq.track === 'function' && broker?.pixel_tiktok) {
        window.ttq.track('Contact', { content_name: propiedad.titulo, content_id: propiedad.id, value: propiedad.precio, currency: moneda });
      }
    } catch (err) { console.error('Error silencioso en píxeles Elite4', err); }
    ...
    <div class="p-8 lg:p-12 flex-1 flex flex-col">
      ...
      <div class="mt-auto pt-8 border-t border-white/10">
        ...
        <form method="POST" action="?/contacto" use:enhance={() => { 
          ...
          // 🔥 TRACKING ENTERPRISE DEL FORMULARIO ELITE4 (Split View)
          try {
            if (typeof window.fbq === 'function' && broker?.pixel_fb) {
            ...
            console.log('🔥 Formulario Elite4: Lead Disparado');
          ...
          {#if broker.whatsapp}
            <a 
              href="https://wa.me/{broker.whatsapp}?text=Hola,%20me%20interesa%20agendar%20un%20recorrido%20para:%20{propiedad.titulo}" 
              target="_blank" 
              class="w-full h-14 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-white bg-[#25D366]/10 border border-[#25D366]/30 hover:bg-[#25D366]/20 transition-colors flex items-center justify-center gap-2 mt-3"
              onclick={registrarLeadWhatsAppSecundario}
            >
