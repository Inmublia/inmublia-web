<script>
  ...
  import { 
    ...
    Linkedin, CheckCircle2, AlertCircle, Sparkles, BedDouble, 
    Bath, Maximize, Car, MapPin
  } from 'lucide-svelte';
...
<main class="min-h-screen {isNight ? 'bg-zinc-950 text-zinc-50' : 'bg-slate-50 text-slate-900'} font-sans pb-32 transition-colors duration-1000">
  ...
  <nav class="fixed top-0 w-full z-40 backdrop-blur-xl border-b {isNight ? 'bg-zinc-950/80 border-zinc-800' : 'bg-white/80 border-slate-200'}">
    <div class="max-w-[1400px] mx-auto px-6 h-20 flex justify-between items-center">
      ...
      <a href="https://{broker.subdominio}.inmublia.com" class="font-bold text-xs uppercase tracking-widest {isNight ? 'text-zinc-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'} transition-colors flex items-center gap-2">Catálogo Exclusivo</a>
    </div>
  </nav>
  ...
  <div class="w-full pt-20">
    <div class="w-full flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-4 px-4 md:px-8 py-6">
      {#each allPhotos as foto, idx}
        <div class="shrink-0 w-[85vw] md:w-[65vw] lg:w-[55vw] h-[45vh] md:h-[60vh] snap-center relative rounded-[2rem] overflow-hidden cursor-pointer shadow-lg {isNight ? 'border border-zinc-800 bg-zinc-900' : 'border border-slate-200 bg-white'} group" role="button" tabindex="0" onclick={() => openGallery(idx)}>
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
      <div class="flex-1">
        <div class="flex items-center gap-3 mb-4 flex-wrap">
          <span class="text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-md {isNight ? 'bg-zinc-800 text-zinc-300' : 'bg-slate-200 text-slate-700'}">{propiedad.operacion}</span>
          {#if propiedad.destacada}
            <span class="text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-md bg-amber-500/10 text-amber-600 border border-amber-500/20 flex items-center gap-1"><Sparkles class="w-3 h-3" /> Exclusiva</span>
          {/if}
          <span class="text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 {isNight ? 'text-zinc-500' : 'text-slate-400'}"><MapPin class="w-3 h-3" /> {propiedad.ubicacion}</span>
        </div>
        <h1 class="text-4xl md:text-5xl font-black tracking-tight leading-tight mb-2 {isNight ? 'text-white' : 'text-slate-900'}">{propiedad.titulo}</h1>
      </div>
      <div class="text-left md:text-right shrink-0">
        ...
        <p class="text-3xl md:text-4xl font-light {isNight ? 'text-white' : 'text-slate-900'}">{formatearPrecio(propiedad.precio, moneda)}</p>
      </div>
    </div>

    <div class="flex flex-wrap justify-center gap-4 mb-16">
      <div class="flex-1 min-w-[120px] max-w-[200px] flex flex-col items-center justify-center p-6 rounded-3xl shadow-sm transition-transform hover:-translate-y-1 {isNight ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-slate-200'}">
      ...
      {#if propiedad.medio_bano > 0}
        <div class="flex-1 min-w-[120px] max-w-[200px] flex flex-col items-center justify-center p-6 rounded-3xl shadow-sm transition-transform hover:-translate-y-1 {isNight ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-slate-200'}">
          <Bath class="w-6 h-6 mb-3 {isNight ? 'text-zinc-500' : 'text-slate-400'}" />
          <span class="text-2xl font-semibold {isNight ? 'text-zinc-100' : 'text-slate-900'}">{propiedad.medio_bano}</span>
          <span class="text-[9px] font-bold uppercase tracking-widest mt-1 {isNight ? 'text-zinc-500' : 'text-slate-400'}">Medio Baño</span>
        </div>
      {/if}
      <div class="flex-1 min-w-[120px] max-w-[200px] flex flex-col items-center justify-center p-6 rounded-3xl shadow-sm transition-transform hover:-translate-y-1 {isNight ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-slate-200'}">
      ...
      {#if propiedad.m2_terreno > 0}
        <div class="flex-1 min-w-[120px] max-w-[200px] flex flex-col items-center justify-center p-6 rounded-3xl shadow-sm transition-transform hover:-translate-y-1 {isNight ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-slate-200'}">
          <Maximize class="w-6 h-6 mb-3 {isNight ? 'text-zinc-500' : 'text-slate-400'}" />
          <span class="text-2xl font-semibold {isNight ? 'text-zinc-100' : 'text-slate-900'}">{propiedad.m2_terreno}</span>
          <span class="text-[9px] font-bold uppercase tracking-widest mt-1 {isNight ? 'text-zinc-500' : 'text-slate-400'}">M² Terreno</span>
        </div>
      {/if}
      <div class="flex-1 min-w-[120px] max-w-[200px] flex flex-col items-center justify-center p-6 rounded-3xl shadow-sm transition-transform hover:-translate-y-1 {isNight ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-slate-200'}">
