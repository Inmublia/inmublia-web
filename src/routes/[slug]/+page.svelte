<script>
  ...
  // Cerebro SaaS: ¿Qué plantilla eligió el admin?
...
{#if isGalleryOpen}
  <div class="fixed inset-0 z-[200] bg-black/95 backdrop-blur-xl flex items-center justify-center" role="dialog" aria-modal="true" tabindex="-1" onclick={closeGallery}>
    ...
    <div class="relative max-w-7xl max-h-[90vh] px-16 flex items-center justify-center w-full h-full" role="document" tabindex="0" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/50 px-4 py-2 rounded-full text-white text-xs font-bold uppercase backdrop-blur-md">{currentImageIndex + 1} / {allPhotos.length}</div>
    </div>
...
{#if !isBrochure}
  <a href="https://wa.me/{broker.whatsapp}?text=Hola..." target="_blank" class="fixed bottom-8 right-8 md:bottom-28 md:right-8 bg-[#25D366] text-white p-4 rounded-full shadow-lg z-[100]" aria-label="WhatsApp"><svg class="w-9 h-9" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg></a>
  <div class="fixed bottom-0 left-0 w-full z-50 bg-white/90 backdrop-blur-xl border-t border-slate-200 py-3 px-6 md:px-12 shadow-lg">
    <div class="max-w-[1400px] mx-auto flex items-center justify-between">
      <div><p class="text-[10px] font-bold uppercase text-slate-500">{propiedad.operacion}</p><p class="text-base md:text-xl font-black text-slate-900 leading-none mt-1">{formatearPrecio(propiedad.precio)}</p></div>
      <div class="flex items-center gap-4">
        <div class="w-10 h-10 rounded-full bg-slate-900 overflow-hidden shrink-0 border border-slate-200">{#if broker.avatar_url} <img src={broker.avatar_url} alt="Asesor" class="w-full h-full object-cover"> {/if}</div>
        <div class="hidden sm:block text-right"><p class="text-xs font-bold text-slate-900 uppercase">{broker.nombre_comercial}</p><p class="text-[9px] text-slate-500 uppercase tracking-widest">Asesor Privado</p></div>
      </div>
    </div>
  </div>
{/if}
...
{#snippet classic()}
  <main class="min-h-screen bg-white font-sans text-slate-900 pb-32 pt-10">
    <div class="max-w-4xl mx-auto px-6 text-center">
      <h1 class="text-3xl font-black mb-2">{propiedad.titulo}</h1>
      <p class="text-slate-500 mb-8">{propiedad.ubicacion}</p>
      <img src={propiedad.imagen_url} alt="Hero" class="w-full rounded-2xl cursor-pointer shadow-md mb-8" tabindex="0" onclick={() => openGallery(0)}>
      <h2 class="text-2xl font-bold text-indigo-600 mb-8">{formatearPrecio(propiedad.precio)}</h2>
      <div class="prose mx-auto text-left mb-12">{propiedad.descripcion}</div>
    </div>
  </main>
{/snippet}

{#snippet modern()}
  <main class="min-h-screen bg-slate-50 font-sans text-slate-900 pb-32">
    <div class="max-w-6xl mx-auto px-6 pt-12 flex flex-col md:flex-row gap-8">
      <div class="flex-1">
        <h1 class="text-4xl font-bold mb-6">{propiedad.titulo}</h1>
        <img src={propiedad.imagen_url} alt="Hero" class="w-full h-[500px] object-cover rounded-3xl cursor-pointer shadow-lg mb-8" tabindex="0" onclick={() => openGallery(0)}>
        <p class="text-lg text-slate-700 leading-relaxed bg-white p-8 rounded-3xl shadow-sm">{propiedad.descripcion}</p>
      </div>
      <div class="w-full md:w-80">
        <div class="bg-white p-6 rounded-3xl shadow-xl sticky top-6">
          <h2 class="text-2xl font-black mb-4">{formatearPrecio(propiedad.precio)}</h2>
          <p class="text-sm text-slate-500 mb-6">{propiedad.ubicacion}</p>
          <button class="w-full bg-slate-900 text-white font-bold py-3 rounded-xl">Contactar</button>
        </div>
      </div>
    </div>
  </main>
            ...
            <div class="flex flex-col gap-10 md:gap-16">
              {#each propiedad.galeria_urls as foto, idx}
                <div class="w-full rounded-sm overflow-hidden shadow-xl {isNight ? 'bg-slate-800 shadow-black/50' : 'bg-slate-100 shadow-slate-200/50'} relative group cursor-pointer" role="button" tabindex="0" onclick={() => openGallery(idx + 1)}>
                  <img src={foto} alt="Detalle de la propiedad" class="w-full h-auto max-h-[80vh] object-cover transition-transform duration-700 ease-out group-hover:scale-[1.02]" loading="lazy">
          ...
          {#if propiedad.recorrido_3d_url && obtenerIdMatterport(propiedad.recorrido_3d_url)}
            <div>
              ...
              <div class="relative w-full pb-[56.25%] h-0 rounded-sm overflow-hidden shadow-xl border border-slate-100 bg-black">
                <iframe title="Matterport" src="https://my.matterport.com/show/?m={obtenerIdMatterport(propiedad.recorrido_3d_url)}" class="absolute top-0 left-0 w-full h-full border-0" allowfullscreen allow="xr-spatial-tracking"></iframe>
              </div>
            </div>
          {/if}
      ...
      {#if !isBrochure}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-24">
          <div class="{isNight ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border rounded-2xl p-8 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            {#if broker.bio}<p class="text-xs text-slate-500 italic mb-6">"{broker.bio}"</p>{/if}
            <button onclick={descargarVCard} aria-label="Descargar tarjeta de contacto" class="inline-flex items-center gap-2 px-6 py-3 {isNight ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-900 hover:bg-slate-800'} text-white text-[10px] font-bold uppercase tracking-widest rounded-full transition-colors shadow-sm w-full justify-center">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg> Guardar Contacto
            </button>
          </div>
          ...
          <div class="{isNight ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'} border rounded-2xl p-8 shadow-sm flex flex-col justify-center">
            ...
            {#if form?.success}
              <div class="bg-emerald-50 border border-emerald-200 text-emerald-600 font-bold p-4 rounded-xl text-sm text-center" role="alert">Solicitud enviada con éxito. El asesor se comunicará pronto.</div>
            {:else if form?.error}
              <div class="mb-4 bg-red-50 text-red-700 font-bold p-3 rounded-lg text-sm border border-red-200 text-center" role="alert">{form.error}</div>
            {/if}
            <form method="POST" action="?/contacto" use:enhance={() => { enviando = true; return async ({ update }) => { enviando = false; update({ reset: form?.success }); }; }} class="space-y-4">
              ...
              <div><label for="nombre" class="sr-only">Nombre completo</label><input type="text" id="nombre" name="nombre" required class="w-full {isNight ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'} border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Nombre completo"></div>
              <div><label for="correo" class="sr-only">Correo electrónico</label><input type="email" id="correo" name="correo" required class="w-full {isNight ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'} border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Correo electrónico"></div>
              <div><label for="telefono" class="sr-only">Teléfono móvil</label><input type="tel" id="telefono" name="telefono" required class="w-full {isNight ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'} border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Teléfono móvil"></div>
              <button type="submit" disabled={enviando} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg text-sm transition-all disabled:opacity-50">{enviando ? 'Enviando...' : 'Solicitar Información'}</button>
            </form>
          </div>
        </div>
      {/if}
...
{#if plantillaActiva === 'modern'}
  {@render modern()}
{:else if plantillaActiva === 'classic'}
  {@render classic()}
{:else}
  {@render luxury()}
{/if}
