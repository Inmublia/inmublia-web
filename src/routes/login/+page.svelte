...
<div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
  <div class="w-full max-w-md relative z-10">
    <div class="text-center mb-10">
      ...
      <h2 class="mt-6 text-2xl font-black text-slate-900 tracking-tight">Ingresa a tu Consola</h2>
      <p class="text-slate-500 mt-2 font-medium">Gestiona tu agencia y tu inventario</p>
    </div>
    ...
    <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl shadow-slate-200/50">
      ...
      <!-- Formulario conectado a la acción "ingresar" del servidor -->
      <form method="POST" action="?/ingresar" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; update(); }; }} class="space-y-6">
        ...
        <div>
          <label for="password" class="block text-sm font-bold text-slate-700 mb-2 flex justify-between">
            Contraseña
            <a href="#" class="text-blue-600 hover:text-blue-800 text-xs">¿Olvidaste tu contraseña?</a>
          </label>
          <input 
            type="password" 
            id="password" 
