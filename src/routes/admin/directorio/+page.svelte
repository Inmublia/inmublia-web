...
<div class="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
  ...
  <header class="bg-zinc-950 text-white pt-10 pb-16 px-6 sm:px-12 shrink-0 relative overflow-hidden shadow-2xl shadow-zinc-900/20">
  ...
  <main class="flex-1 w-full max-w-[1400px] mx-auto px-4 sm:px-12 py-8 relative z-20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
      ...
      <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col justify-between">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="w-10 h-10 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-600 border border-slate-200/50">
            <Users class="w-5 h-5" />
          </div>
        </div>
      ...
      <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col justify-between">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="w-10 h-10 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 border border-indigo-100/50 relative">
      ...
      <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col justify-between">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="w-10 h-10 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 border border-emerald-100/50">
    ...
    <div class="space-y-4">
      {#each clientesInteligentes() as cliente}
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col lg:flex-row transition-all hover:shadow-md hover:border-slate-300">
          
          <div class="flex-1 p-6 border-b lg:border-b-0 lg:border-r border-slate-100 flex gap-5">
            <div class="w-14 h-14 rounded-full bg-slate-100 shrink-0 shadow-inner border border-slate-200 overflow-hidden hidden sm:block">
            ...
            <div class="flex-1">
              <div class="flex items-center justify-between gap-4 mb-2">
                ...
                <button onclick={() => enviarWhatsApp(cliente.telefono, cliente.nombre, null)} class="shrink-0 w-8 h-8 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-colors" title="Chatear con el cliente">
              ...
              <p class="text-[11px] font-semibold text-slate-500 mb-4 font-mono">{cliente.telefono} • {cliente.correo}</p>
              
              <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Intereses Previos ({cliente.interesesHistorial.length})</p>
                <div class="flex flex-wrap gap-2">
                  {#each cliente.interesesHistorial as interes}
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-bold text-slate-600 shadow-sm truncate max-w-[200px]">
                ...
                <div class="mt-4 flex items-center justify-between border-t border-slate-200/60 pt-3">
                  <span class="text-xs font-bold text-slate-500">Presupuesto Calculado:</span>
          ...
          <div class="w-full lg:w-[380px] bg-slate-50/50 p-6 shrink-0 flex flex-col justify-center relative">
            {#if cliente.matches.length > 0}
              ...
              <div class="flex items-center justify-between mb-4 relative z-10">
                <p class="text-[10px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-1.5">
                  <Sparkles class="w-3.5 h-3.5" /> Oportunidad de Venta
                </p>
                <span class="bg-amber-100 text-amber-800 text-[10px] font-black px-2 py-1 rounded-md shadow-sm border border-amber-200">
                  +{cliente.matches.length} matches
                </span>
              </div>

              <div class="bg-white rounded-2xl p-3 border border-amber-200/60 shadow-sm mb-4 relative z-10 flex gap-4 items-center">
                <div class="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 shrink-0">
                  <img src={bestMatch.imagen_url} alt="Match" class="w-full h-full object-cover">
                </div>
            ...
            {:else}
              <div class="flex flex-col items-center justify-center text-center opacity-60 h-full py-6">
                <div class="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-400 mb-2">
                  <Search class="w-4 h-4" />
                </div>
                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Sin Coincidencias</p>
                <p class="text-[10px] text-slate-400 font-medium mt-1">Inventario incompatible.</p>
              </div>
            {/if}
          </div>
