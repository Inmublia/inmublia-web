    ...
    <div class="w-full max-w-[1400px] mx-auto relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      <div class="flex items-center gap-3">
        {#if broker && broker.subdominio}
          <a href="https://{broker.subdominio}.inmublia.com" target="_blank" data-sveltekit-reload class="hidden sm:inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-semibold transition-colors border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 text-zinc-300 hover:text-white h-11 px-5 gap-2 backdrop-blur-sm">
            <ExternalLink class="w-4 h-4" /> Ver Portal Público
          </a>
        {:else if broker && !broker.subdominio}
          <a href="/admin/perfil" data-sveltekit-reload class="hidden sm:inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-semibold transition-colors border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 hover:text-amber-400 h-11 px-5 gap-2 backdrop-blur-sm">
