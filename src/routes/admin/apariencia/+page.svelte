...
<script>
  ...
  // Recibimos el slug seguro desde el server
  let primeraPropiedadSlug = $derived(data.previewSlug);
    ...
    <div class="max-w-6xl mx-auto space-y-12">
      ...
      <div class="bg-slate-900 p-8 rounded-3xl shadow-xl border border-slate-800 relative overflow-hidden">
        ...
        <div class="mb-8 border-b border-slate-800 pb-4 relative z-10">
          ...
          <p class="text-xs font-medium text-slate-400 mt-1">Estos diseños se aplican de forma individual en cada propiedad de su inventario. (Modo exhibición).</p>
        </div>
            ...
            <div class="relative bg-slate-800/50 border border-slate-700 rounded-2xl flex flex-col overflow-hidden {!autorizado ? 'opacity-40 grayscale' : 'hover:border-indigo-400 hover:shadow-lg transition-all group'}">
              ...
              <div class="h-20 bg-slate-950 flex flex-col items-center justify-center relative p-3">
                ...
                {#if primeraPropiedadSlug && autorizado}
                  <a href="https://{broker.subdominio}.inmublia.com/{primeraPropiedadSlug}?brochure=true&template={propTemplate.id}" target="_blank" rel="noopener noreferrer" class="absolute inset-0 bg-indigo-900/90 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10 cursor-pointer">
                    <span class="text-white font-bold text-[10px] uppercase tracking-widest border border-white/30 px-4 py-2 rounded-lg hover:bg-white/20 transition-colors flex items-center gap-1.5"><Eye class="w-3.5 h-3.5" /> Ver Demo Real</span>
                  </a>
                {/if}
              </div>
              ...
              <div class="p-5 flex flex-col flex-1">
                ...
                <p class="text-[10px] text-slate-400 font-medium leading-relaxed mb-4">{propTemplate.desc}</p>

                <div class="mt-auto">
                    {#if primeraPropiedadSlug && autorizado}
                      <a href="https://{broker.subdominio}.inmublia.com/{primeraPropiedadSlug}?brochure=true&template={propTemplate.id}" target="_blank" rel="noopener noreferrer" class="flex items-center justify-center gap-1.5 text-[9px] font-bold uppercase tracking-widest bg-slate-800 border border-slate-700 text-slate-300 px-3 py-2 rounded hover:text-white hover:bg-indigo-600 transition-colors w-full text-center">
                        <Eye class="w-3.5 h-3.5" /> Ver Demo Real
                      </a>
                    {:else if !autorizado}
                      <button disabled class="flex items-center justify-center gap-1.5 text-[9px] font-bold uppercase tracking-widest bg-slate-800/50 border border-slate-700/50 text-slate-500 px-3 py-2 rounded cursor-not-allowed w-full text-center">
                        Requiere Plan {propTemplate.minPlan}
                      </button>
                    {:else}
                      <button disabled class="flex items-center justify-center gap-1.5 text-[9px] font-bold uppercase tracking-widest bg-slate-800 border border-slate-700 text-slate-500 px-3 py-2 rounded cursor-not-allowed w-full text-center" title="Añade una propiedad para habilitar las vistas previas reales">
