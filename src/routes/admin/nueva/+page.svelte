<script>
  ...
  // Atrapamos la respuesta del servidor
  let { form } = $props();
  ...
  <main class="flex-1 overflow-auto p-8">
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 p-8 sm:p-10">
      ...
      <div class="mb-8 border-b border-slate-100 pb-6">
        ...
        <p class="text-slate-500 mt-1">Sube la fotografía de alta calidad y completa la información para publicar en la red.</p>
      </div>
      ...
      <form method="POST" action="?/crear" enctype="multipart/form-data" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; update(); }; }} class="space-y-8">
        ...
        <div>
          ...
          <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:border-blue-500 hover:bg-blue-50 transition-colors relative overflow-hidden group">
            ...
            {#if imagePreview}
              <img src={imagePreview} alt="Vista previa de la propiedad" class="absolute inset-0 w-full h-full object-cover z-10" />
        ...
        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
          <div class="sm:col-span-2">
            ...
            <input type="text" name="titulo" id="titulo" value={form?.titulo || ''} required placeholder="Ej. Residencia de Lujo en Zapopan" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all">
          </div>
          ...
          <div class="sm:col-span-2">
            ...
            <div class="relative rounded-xl shadow-sm">
              ...
              <input type="number" name="precio" id="precio" value={form?.precio || ''} required placeholder="5000000" class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-4 py-3 text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all">
            </div>
          </div>

          <div class="sm:col-span-2">
            <label for="descripcion" class="block text-sm font-bold text-slate-700 mb-2">Descripción Completa</label>
            <textarea id="descripcion" name="descripcion" rows="4" placeholder="Describe los acabados, ubicación y ventajas de la propiedad..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all">{form?.descripcion || ''}</textarea>
          </div>
        </div>
