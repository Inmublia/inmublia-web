...
<script>
  ...
  // 🚀 ESTADO DE TABS PARA EL COMMAND CENTER
...
<main class="flex-1 flex flex-col h-screen overflow-hidden relative bg-[#F8FAFC] font-sans text-slate-900">
  ...
  <div class="relative flex-1 flex overflow-hidden group z-20 -mt-16 w-full">
    ...
    <div class="flex-1 overflow-x-auto overflow-y-hidden kanban-board px-4 sm:px-8 pb-6 transition-opacity" bind:this={boardContainer}>
  ...
  <!-- 🚀 COMMAND CENTER WORKSPACE A PANTALLA COMPLETA (Propuesta B+D) -->
  {#if selectedLead}
    <div class="fixed inset-0 z-[110] bg-slate-50 flex flex-col animate-[fadeIn_0.2s_ease-out]">
      ...
      <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 shadow-sm z-10">
        <div class="flex items-center gap-6">
          ...
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-full border-2 border-slate-100 shadow-sm bg-slate-800 text-white flex items-center justify-center text-sm font-black uppercase">
        ...
        <div class="flex items-center gap-4">
          <button onclick={() => pedirEliminarLead(selectedLead)} class="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-full transition-colors" title="Borrar Lead">
            <Trash2 class="w-5 h-5"/>
          </button>
          <div class="w-px h-8 bg-slate-200 mx-2"></div>
          <div class="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-lg text-amber-700 shadow-inner">
            <Sparkles class="w-4 h-4 text-amber-500" />
            <span class="text-[10px] font-black tracking-widest uppercase">{creditosIA} Créditos IA</span>
          </div>
        </div>
      </header>
      ...
      <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="w-full md:w-[400px] xl:w-[450px] bg-slate-50 border-r border-slate-200 flex flex-col overflow-y-auto shrink-0 shadow-[inset_-10px_0_20px_rgba(0,0,0,0.02)]">
          
          <!-- Score Inmublia -->
          {#if selectedLead.scoreObj && selectedLead.estado !== 'cerrado' && selectedLead.estado !== 'descartado'}
            <div class="p-6 border-b border-slate-200 bg-white">
              <div class="flex justify-between items-end mb-4">
                <h2 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Inmublia Score</h2>
                <span class="text-4xl font-black tracking-tighter {selectedLead.scoreObj.isHot ? 'text-orange-500' : 'text-slate-900'}">{selectedLead.scoreObj.score}<span class="text-lg text-slate-300">/100</span></span>
              </div>
              
              <div class="{selectedLead.scoreObj.isHot ? 'bg-orange-50 border-orange-200 text-orange-900' : 'bg-indigo-50/50 border-indigo-100 text-indigo-900'} border rounded-xl p-4 shadow-sm">
                <h3 class="text-sm font-bold mb-1 flex items-center gap-2">
                  <Flame class="w-4 h-4 {selectedLead.scoreObj.isHot ? 'text-orange-500' : 'text-indigo-500'}" />
                  {selectedLead.scoreObj.etiqueta}
                </h3>
                <p class="text-xs opacity-80 leading-relaxed font-medium mb-3">
                  {selectedLead.scoreObj.razon}
                </p>
                <div class="bg-white/60 rounded-lg p-2.5 border border-white flex items-start gap-2">
                  <ArrowRight class="w-4 h-4 opacity-70 shrink-0 mt-0.5" />
                  <p class="text-xs font-bold leading-snug">{selectedLead.scoreObj.accion}</p>
                </div>
              </div>
            </div>
          {/if}
          ...
          <!-- Propiedad Anclada -->
          <div class="p-6 border-b border-slate-200 bg-white">
            <h2 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Propiedad de Interés</h2>
              ...
              <div class="group relative rounded-xl overflow-hidden border border-slate-200 shadow-sm hover:border-indigo-300 transition-colors cursor-pointer">
                <div class="aspect-[21/9] bg-slate-100 relative">
                  ...
                  <div class="absolute bottom-3 left-3 right-3 text-white">
                    ...
                    <p class="text-[9px] font-black tracking-widest opacity-80 uppercase mt-0.5">{selectedLead.propiedades.ubicacion}</p>
                  </div>
                </div>
                <div class="p-3 bg-slate-50 flex justify-between items-center border-t border-slate-100">
                  <span class="text-xs font-black text-slate-900">{formatMoney(selectedLead.propiedades.precio)} MXN</span>
                  <div class="bg-white rounded p-1 border border-slate-200 shadow-sm group-hover:border-indigo-200 transition-colors">
                    <Home class="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-600" />
                  </div>
                </div>
              </div>
            {:else}
              <div class="p-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center">
                <Building2 class="w-6 h-6 text-slate-300 mx-auto mb-2" />
                <p class="text-xs text-slate-500 font-medium">Búsqueda general del inventario.</p>
              </div>
            {/if}
          </div>
          ...
          <div class="p-6 flex-1 bg-transparent">
            ...
            <div class="space-y-3">
              {#each (selectedLead.lead_notas || []).filter(n => n.tipo === 'recordatorio' && !n.completado) as nota}
                <div class="flex items-start gap-3 p-3.5 rounded-xl border {isOverdue(nota.fecha_recordatorio) ? 'border-rose-300 bg-rose-50 shadow-rose-500/10' : 'border-amber-200 bg-amber-50 shadow-amber-500/10'} shadow-sm transition-all group">
                  <button onclick={() => completarRecordatorio(nota.id)} class="mt-0.5 shrink-0 w-5 h-5 rounded-md border-2 {isOverdue(nota.fecha_recordatorio) ? 'border-rose-400 text-rose-500 hover:bg-rose-100' : 'border-amber-400 text-amber-500 hover:bg-amber-100'} flex items-center justify-center transition-colors">
                    <CheckCircle2 class="w-3.5 h-3.5 opacity-0 hover:opacity-100 transition-opacity" />
                  </button>
                  <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold text-slate-900 leading-snug">{nota.contenido}</p>
                    <p class="text-[10px] font-bold uppercase tracking-widest mt-1.5 flex items-center gap-1 {isOverdue(nota.fecha_recordatorio) ? 'text-rose-600' : 'text-amber-600'}">
                      <Clock class="w-3 h-3" /> {formatDateTime(nota.fecha_recordatorio)}
                    </p>
                  </div>
                </div>
              {/each}
        ...
        <!-- COLUMNA DERECHA: Ejecución y Bitácora (60%) -->
        <div class="flex-1 flex flex-col bg-white overflow-hidden relative">
          
          <!-- Copiloto IA -->
          <div class="p-6 lg:p-8 border-b border-slate-200 bg-gradient-to-b from-indigo-50/50 to-white shrink-0">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-xs font-black text-indigo-900 uppercase tracking-widest flex items-center gap-2">
                <Sparkles class="w-4 h-4 text-indigo-600" /> Copiloto Inteligente
              </h2>
              ...
              <form id="form-whatsapp-ia" method="POST" action="?/generarScriptWhatsapp" use:enhance={() => {
                ...
                <button 
                  ...
                  class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-lg transition-colors flex items-center gap-2 shadow-[0_4px_15px_rgba(79,70,229,0.3)] active:scale-95"
                >
                  {#if iaGenerandoWs}
            ...
            <div class="relative">
              {#if iaWsError}
                <div class="absolute inset-x-0 -top-2 transform -translate-y-full mb-2 z-10 bg-rose-50 text-rose-700 text-xs p-3 rounded-lg border border-rose-200 font-medium flex items-center gap-2 shadow-sm animate-[fadeIn_0.2s_ease-out]">
                  <AlertTriangle class="w-4 h-4"/> {iaWsError}
                  <button onclick={() => iaWsError = ''} class="ml-auto"><X class="w-3.5 h-3.5"/></button>
                </div>
              {/if}
              
              <textarea 
                bind:value={iaWsGenerado}
                placeholder="El asistente de IA redactará el seguimiento perfecto basado en el historial..."
                class="w-full h-28 sm:h-32 p-4 rounded-xl border border-indigo-100 bg-white text-slate-700 text-sm font-medium resize-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-inner"
              ></textarea>
              
              <div class="absolute bottom-3 right-3 flex items-center gap-2">
                {#if iaWsGenerado}
                  <button onclick={() => iaWsGenerado = ''} class="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="Descartar">
                    <X class="w-4 h-4"/>
                  </button>
                  <a href="https://wa.me/{selectedLead.telefono?.replace(/\D/g, '') || ''}?text={encodeURIComponent(iaWsGenerado)}" target="_blank" rel="noopener noreferrer" class="px-5 py-2 bg-[#25D366] hover:bg-[#128C7E] text-white text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 transition-all shadow-md shadow-[#25D366]/30 active:scale-95">
          ...
          <!-- Bitácora y Comunicación -->
          <div class="flex flex-col flex-1 overflow-hidden">
            <div class="flex border-b border-slate-200 px-8 shrink-0 bg-slate-50/80">
              <button class="px-4 py-4 text-[10px] font-black uppercase tracking-[0.2em] border-b-2 {activeTab === 'timeline' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'} transition-colors" onclick={() => activeTab = 'timeline'}>
                Timeline Crónologico
              </button>
            </div>

            <!-- Input Rápido (Fijo arriba de la bitácora) -->
            <div class="p-6 bg-white border-b border-slate-100 shrink-0">
              <form method="POST" action="?/guardarNota" use:enhance={manejadorNota} class="flex flex-col gap-3">
                ...
                <div class="flex items-center gap-1.5 p-1 bg-slate-50 border border-slate-200 rounded-lg w-fit shadow-inner">
                  <button type="button" onclick={() => esRecordatorio = false} class="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-md transition-all flex items-center gap-1.5 {!esRecordatorio ? 'bg-white shadow-sm border border-slate-200 text-slate-900' : 'text-slate-500 hover:text-slate-900'}">
                    <MessageSquareQuote class="w-3 h-3"/> Minuta
                  </button>
                  <button type="button" onclick={() => esRecordatorio = true} class="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-md transition-all flex items-center gap-1.5 {esRecordatorio ? 'bg-amber-500 shadow-sm text-white' : 'text-slate-500 hover:text-slate-900'}">
                ...
                {#if esRecordatorio}
                  <div class="flex gap-3 animate-[fadeIn_0.2s_ease-out]">
                    <input type="date" lang="es-MX" bind:value={fechaRecordatorio} class="flex-1 bg-white border border-amber-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 focus:ring-2 focus:ring-amber-500/30 outline-none shadow-sm">
                    <input type="time" lang="es-MX" bind:value={horaRecordatorio} class="flex-1 bg-white border border-amber-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 focus:ring-2 focus:ring-amber-500/30 outline-none shadow-sm">
                  </div>
                {/if}

                <div class="relative flex items-end gap-3 bg-white border border-slate-200 rounded-xl p-2 shadow-sm focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
                  <textarea name="contenido" bind:value={nuevaNotaTexto} onkeydown={handleKeyDown} placeholder={esRecordatorio ? "Describe la acción a agendar..." : "Registra una llamada o actualización rápida..."} class="flex-1 bg-transparent border-none outline-none text-sm text-slate-700 placeholder-slate-400 resize-none min-h-[40px] px-2 py-1 font-medium" required></textarea>
                  <button type="submit" bind:this={submitBtn} disabled={guardandoNota || !nuevaNotaTexto.trim()} class="w-10 h-10 shrink-0 flex items-center justify-center rounded-lg {esRecordatorio ? 'bg-amber-500 hover:bg-amber-600' : 'bg-slate-900 hover:bg-indigo-600'} text-white transition-colors disabled:opacity-50 disabled:bg-slate-300">
                    {#if guardandoNota}
            ...
            <div class="flex-1 overflow-y-auto p-8 bg-slate-50/50">
              {#if selectedLead.lead_notas && selectedLead.lead_notas.length > 0}
                <div class="space-y-6 relative before:absolute before:inset-0 before:ml-[17px] before:h-full before:w-px before:bg-slate-200">
                  {#each selectedLead.lead_notas as nota}
                    <div class="relative flex items-start gap-6">
                      ...
                      <div class="ml-14 bg-white border border-slate-200 rounded-xl p-4 shadow-sm w-full transition-shadow hover:shadow-md">
                        ...
                        <p class="text-sm text-slate-700 font-medium whitespace-pre-wrap leading-relaxed {nota.completado ? 'line-through opacity-70' : ''}">{nota.contenido}</p>
                        
                        {#if nota.tipo === 'recordatorio' && !nota.completado && !nota.id.startsWith('temp-')}
                          <div class="flex justify-end pt-3 mt-2 border-t border-slate-50">
                            <button onclick={() => completarRecordatorio(nota.id)} class="text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-200 transition-colors active:scale-95 text-slate-600">
                              <CheckCircle2 class="w-3.5 h-3.5"/> Marcar como Resuelto
                            </button>
                          </div>
                        {/if}
                      </div>
              ...
              {:else}
                <div class="flex flex-col items-center justify-center h-full text-center opacity-70">
                  ...
                  <p class="text-sm font-bold text-slate-500">Aún no hay actividad.</p>
                  <p class="text-xs text-slate-400 mt-1">Registra tu primera interacción arriba.</p>
                </div>
              {/if}
            </div>
          </div>
