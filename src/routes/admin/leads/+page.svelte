...
<script>
  ...
  // 🚀 FIX: Pipeline Financiero Dinámico (Problema 5)
  ...
  // 🚀 FIX: Semántica de Urgencia Real (Problema 4)
    ...
    <div class="flex-1 overflow-x-auto overflow-y-hidden kanban-board px-4 sm:px-8 pb-6 transition-opacity {isPanelOpen ? 'pointer-events-none select-none opacity-50' : ''}" bind:this={boardContainer}>
      <div class="flex gap-4 items-start h-full min-w-max xl:min-w-full">
        ...
        {#each columnas as columna}
          ...
          >
            <!-- 🚀 HEADER DE COLUMNA Y PIPELINE FINANCIERO -->
            <div class="sticky top-0 bg-transparent z-10 pt-1 pb-2">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-[9px] font-black px-2 py-0.5 rounded-md bg-white/90 border {columna.border} {columna.text} shadow-sm">
                  {metricasColumnas[columna.id].count}
                </span>
              </div>
              
              {#if metricasColumnas[columna.id].totalValue > 0}
                <div class="text-[9px] font-bold text-slate-400 px-0.5">
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar flex flex-col gap-2 pb-8 pt-1">
              {#each leadsPorColumna[columna.id] || [] as lead (lead.id)}
                ...
                >
                  ...
                  <!-- 🚀 PROPIEDAD: COMPACTA Y DE ALTO CONTRASTE -->
                  ...
                  <!-- 🚀 ACCIONES RÁPIDAS EN HOVER (CRISTAL) -->
                  <div class="absolute -top-3 -right-2 opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-center gap-1 bg-white/95 backdrop-blur-sm p-1 rounded-xl shadow-lg border border-slate-200 z-10 scale-95 group-hover:scale-100 origin-bottom-right">
                    {#if lead.telefono}
    ...
    {#if selectedLead}
        ...
        {#if selectedLead.telefono}
          <div class="flex flex-col items-center sm:items-end gap-2 w-full sm:w-auto">
             <div class="flex flex-col sm:flex-row items-center justify-end gap-2 w-full sm:w-auto">
               ...
               <a href="https://wa.me/{String(selectedLead.telefono || '').replace(/\D/g, '')}" target="_blank" rel="noopener noreferrer" class="text-[10px] font-black uppercase tracking-widest text-white bg-[#25D366] hover:bg-[#128C7E] px-4 py-2.5 rounded-xl transition-all shadow-md shadow-[#25D366]/20 flex items-center justify-center gap-2 w-full sm:w-auto active:scale-95">
      ...
      {#if iaWsGenerado || iaWsError || iaGenerandoWs}
        <div class="px-8 pt-4 pb-5 bg-slate-50/50 border-b border-slate-100 shadow-inner animate-[fadeIn_0.3s_ease-out]">
          {#if iaWsError}
             <div class="bg-amber-50 text-amber-700 text-xs p-3 rounded-xl border border-amber-200 font-medium flex items-start gap-2 shadow-sm">
