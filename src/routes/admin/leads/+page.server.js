...
export const actions = {
  actualizar: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase
      .from('leads').update({ estado }).eq('id', id).eq('broker_id', locals.user.id);
  ...
  eliminar: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase
      .from('leads').delete().eq('id', id).eq('broker_id', locals.user.id);
  ...
  guardarNota: async ({ request, locals }) => {
    ...
    const { data: lead, error: checkError } = await locals.supabase
      .from('leads').select('id, estado').eq('id', leadId).eq('broker_id', locals.user.id).maybeSingle();

    if (checkError || !lead) return fail(403, { error: 'No autorizado' });

    const { error: notaError } = await locals.supabase
      .from('lead_notas').insert({ lead_id: leadId, broker_id: locals.user.id, contenido: contenido.trim(), tipo: 'nota' });

    if (notaError) return fail(500, { error: 'Fallo interno' });

    if (lead.estado === 'nuevo') {
      await locals.supabase.from('leads').update({ estado: 'contactado' }).eq('id', leadId).eq('broker_id', locals.user.id);
    }
