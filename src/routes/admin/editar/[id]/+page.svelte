        ...
        <form method="POST" action="?/actualizar" enctype="multipart/form-data" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; update(); }; }} class="space-y-10">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Fotografía Principal</label>
                <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-xl hover:border-blue-500 transition-colors relative overflow-hidden group h-48 bg-slate-50">
                  <img src={imagePreview || propiedad.imagen_url} alt="Portada" class="absolute inset-0 w-full h-full object-cover z-10" />
                  ...
                  <input name="imagen" type="file" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-30" onchange={handleImageChange}>
                </div>
              </div>

              <div>
                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Galería Secundaria</label>
                <div class="flex flex-col items-center justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-xl hover:border-blue-500 transition-colors relative h-48 bg-slate-50 overflow-hidden group">
                  ...
                  {#if galeriaPreviews.length > 0}
                    <div class="absolute inset-0 grid grid-cols-2 gap-1 z-10 p-1">
                      {#each galeriaPreviews.slice(0, 4) as preview}
                        <img src={preview} class="w-full h-full object-cover rounded-md"/>
                      {/each}
                    </div>
                  {:else if propiedad.galeria_urls && propiedad.galeria_urls.length > 0}
                    <div class="absolute inset-0 grid grid-cols-2 gap-1 z-10 p-1">
                      {#each propiedad.galeria_urls.slice(0, 4) as url}
                        <img src={url} class="w-full h-full object-cover rounded-md opacity-70"/>
                      {/each}
                    </div>
                  {:else}
