<script>
  ...
  // FIX: Limpiamos el número de WhatsApp de espacios o guiones para evitar que wa.me crashee
  let whatsappLimpio = $derived(broker?.whatsapp ? String(broker.whatsapp).replace(/\D/g, '') : '');
...
{#if broker && whatsappLimpio}
  <a href="https://wa.me/{whatsappLimpio}" target="_blank" rel="noopener noreferrer" class="fixed bottom-6 right-6 bg-[#25D366] text-white p-4 rounded-full shadow-[0_8px_30px_rgba(37,211,102,0.4)] hover:scale-110 hover:-translate-y-1 transition-all duration-300 z-50 flex items-center justify-center group" aria-label="Contactar por WhatsApp">
